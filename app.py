        ...
        with st.spinner('데이터 분석 및 백테스트 중...'):
            ...
            for y, m in target_ym:
                ...
                if not T_CODE and K_CODE:
                    ...
                    if not km.empty: 
                        history.append({'연도':y,'월':f"{m}월",'날짜':km.index[-1].strftime('%y/%m/%d'),'구분':'평가','종목':K_CODE,'단가':int(km.iloc[-1]),'수량':k_sh,'거래금액':0,'수령배당금':0,'현금잔고':cash,'총자산':cash+(k_sh*int(km.iloc[-1])),'배당률':0.0})
                    ...
                    if t_sh > 0:
                        ...
                        if dt_sw:
                            p_s = int(t_prices_all.loc[dt_sw]); sell_amt = t_sh * p_s; cash += sell_amt
                            ...
                            if dt_sw in k_prices_all.index: 
                                p_k = int(k_prices_all.loc[dt_sw]); k_sh = cash // p_k; cash -= (k_sh * p_k)
                    ...
                    if k_sh > 0:
                        ...
                        if dt_sw:
                            p_s = int(k_prices_all.loc[dt_sw]); sell_amt = k_sh * p_s; cash += sell_amt
                            ...
                            if dt_sw in t_prices_all.index: 
                                p_t = int(t_prices_all.loc[dt_sw]); t_sh = cash // p_t; cash -= (t_sh * p_t)
    ...
    <script>
        ...
        function renderSummary(order) {{
            ...
            tbody.innerHTML = data.map(s => {{
                ...
                return `<tr>
                    <td>${{s.기간}}</td>
                    <td>${{Math.floor(s.주당배당금).toLocaleString()}}</td>
                    <td style='color:#f59e0b; font-weight:600;'>${{parseFloat(s.배당률).toFixed(2)}}%</td>
                    <td>${{fmtMan(s.배당금)}}</td>
                    <td><b>${{fmtMan(s.총자산)}}</b></td>
                    <td style='color:${{colorClass}}; font-weight:600;'>${{fmtMan(s.증감)}}</td>
                </tr>`;
            }}).join('');
        }}
        ...
        function renderHistory(order) {{
            ...
            tbody.innerHTML = data.map(r => {{
                ...
                let amt = r.거래금액 > 0 ? fmtMan(r.거래금액) : '-';
                let div = r.수령배당금 > 0 ? '+' + fmtMan(r.수령배당금) : '-';
                return `<tr${{rowCls}}>
                    <td>${{r.날짜}}</td>
                    <td><span class='badge ${{cls}}'>${{r.구분}}</span></td>
                    <td style='text-align:center;'>${{r.종목}}</td>
                    <td>${{Math.floor(r.단가).toLocaleString()}}</td>
                    <td>${{Math.floor(r.수량).toLocaleString()}}</td>
                    <td>${{amt}}</td>
                    <td class='div-val'>${{div}}</td>
                    <td>${{fmtMan(r.현금잔고)}}</td>
                    <td style='font-weight:700;'>${{fmtMan(r.총자산)}}</td>
                </tr>`;
            }}).join('');
        }}
